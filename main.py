...
#Class Based implementation of the C4.5 Algorithm
...
def c45(dataset: DataSet, org_dataset_info: float,root: Tree):
    ...
    for feature in features:
        ...
        if info == 0:
            leaf_node = Tree(new_subset.headers[new_subset.get_class_index()])
            leaf_node.value = new_subset.rows[0][new_subset.get_class_index()]
...
def main():
    input_list = ['tennis.csv','testSet.csv','testSetSimple.csv','trainSet.csv']
    print("Enter the choice of dataset among the following... \n 1.tennis.csv \n2.testSet.csv \n 3.testSetSimple.csv \n 4.trainSet.csv")
    choice = input("Enter the dataset chosen ")
